      ...
      <form @submit.prevent="postMedicalRecord()" class="registration-form">
        <div class="form-group">
          ...
          <select
            id="patientName"
            class="form-control"
            v-model="newMedicalRecordData.patientID"
            required
          >
        ...
        <div class="form-group">
          ...
          <textarea
            ...
            class="form-control"
            id="newRecordDetails"
            v-model="newMedicalRecordData.RecordDetails"
          ></textarea>
        </div>
      ...
      <form @submit.prevent="updateMedicalRecord()" class="registration-form">
        <div class="form-group">
          ...
          <select
            id="patientName"
            class="form-control"
            v-model="editMedicalRecordData.patientID"
            required
          >
            <option value="" disabled selected>Select Patient</option>
            <option
        ...
        <div class="form-group">
          ...
          <textarea
            ...
            class="form-control"
            id="newRecordDetails"
            v-model="editMedicalRecordData.RecordDetails"
          ></textarea>
        </div>
  ...
  methods: {
    fetchAllMedicalRecords() {
      if (this.isAdmin) {
        ...
        fetch(
          "http://127.0.0.1:8000/api/medical_records?patientID=" + this.user.id,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
    ...
    async postMedicalRecord() {
      try {
        await axios.post(
          "http://127.0.0.1:8000/api/medical_records",
    ...
    async updateMedicalRecord() {
      ...
      await axios.put(
        `http://127.0.0.1:8000/api/medical_records/${this.editMedicalRecordData.id}`,
    ...
    async deleteMedicalRecord(record) {
      ...
      try {
        await axios.delete(
          `http://127.0.0.1:8000/api/medical_records/${record.id}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: "Bearer " + localStorage.getItem("token"),
